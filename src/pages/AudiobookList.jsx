...
//                 </td>
//                 <td className="px-6 py-4 text-gray-300">
//                   {audiobook?.categories[0]?.category}
//                 </td>
...
const AudiobookList = () => {
  ...
  const getCategory = (audiobook) => {
    if (audiobook.category && typeof audiobook.category === "string") {
      return audiobook.category;
    } else if (
      audiobook.categories &&
      Array.isArray(audiobook.categories) &&
      audiobook.categories.length > 0
    ) {
      return audiobook.categories[0]; // Use the first category in the array
    } else {
      return "Uncategorized"; // Default value
    }
  };
          ...
          <tbody>
            {audiobooks.map((audiobook) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-gray-300">
                  {getCategory(audiobook)}
                </td>
                <td className="px-6 py-4 text-center">
                  <button
                    onClick={() => handleDelete(audiobook)}
                    className="inline-flex items-center px-4 py-2 border border-transparent rounded-md text-sm font-medium text-red-600 bg-white hover:bg-red-50 focus:outline-none focus:ring-red-500 ml-2 disabled:opacity-50"
                    disabled={isDeleting}
                  >
                    {isDeleting ? "Deleting..." : "Delete"}
                  </button>
